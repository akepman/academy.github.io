...
(function ($, Drupal, drupalSettings) {
  $( document ).ready(function() {
    $(".feedback-btn").click(function (event) {
      ...
      $.ajax({
        type: "POST",
        url: "/docs" + langPrefix  + "/feedback_vote",
...
(function ($, Drupal) {
  'use strict';
  Drupal.behaviors.ts_feedback = {
    attach: function (context, settings) {
      var feedback = $('#feedback-message', context);
      if(feedback.length) {
        feedback
          .parents('.region')
          .addClass('feedback-container');
        $('.feedback-btn__yes, .feedback-btn__no', context).click(function () {
          var item = $(this);
          item.parents('.region')
            .addClass('feedback-container-comment');
        });

        if(window.scrollY < 30){
          feedback
            .parents('.region')
            .addClass('feedback-container-top');
        }
        ...
        window.addEventListener("scroll", function() {
          if(window.scrollY < 30){
            ...
            feedback
              .parents('.region')
              .removeClass('feedback-container-top');
          }
        });
